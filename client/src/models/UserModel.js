...
const UserModel = {
    ...
    basicDelete: async function basicDelete(url) {
        const token = sessionStorage.getItem("jwt");
        try {
            const response = await fetch(`${API}${url}`, {
                method: 'GET',
            ...
            console.error(`Basic DELETE fetch failed url: ${url}`);
            console.error(error);
            return { error: true };
        }
    },
    ...
    removeAccount: async function removeAccount(id) {
        return await this.basicDelete(`/users/${id}`);
    },

    getTrips: async function getTrips(userId) {
        return await this.basicGet(`/trips/user/${userId}`);
    },
}
